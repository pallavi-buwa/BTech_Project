# Import necessary packages (done) (check up one)
...
cap = cv2.VideoCapture('Export__Jahangir Chowk_2_Friday January 27 202394222  de15ed9_2.mp4')
input_size = 320
...
middle_line_position = 290 
up_line_position = middle_line_position - 15
down_line_position = middle_line_position + 15

ymiddle_line_position = 600   
yup_line_position = ymiddle_line_position - 15
ydown_line_position = ymiddle_line_position + 15
...
def count_vehicle(box_id, img):
    ...
    if (iy > up_line_position) and (iy < middle_line_position) and (ix > 120):
...
def realTime():
    with open("jeh2.csv", 'w') as f1:
        ...
        while True:
            ...
            cropped_image = img[0:50, 850:1240]
            # cv2.imshow("cropped", cropped_image)
    
            # Save the cropped image
            # cv2.imwrite("Cropped Image.jpg", cropped_image)
            text = pytesseract.image_to_string(cropped_image)
            cwriter.writerow(text)
            ...
            cv2.line(img, (ymiddle_line_position,0), (ymiddle_line_position,ih), (255, 0, 255), 2)
            cv2.line(img, (yup_line_position,0), (yup_line_position,ih), (0, 0, 255), 2)
            cv2.line(img, (ydown_line_position,0), (ydown_line_position,ih), (0, 0, 255), 2)
            ...
            cv2.putText(img, "Right", (110, 220), cv2.FONT_HERSHEY_SIMPLEX, font_size, font_color, font_thickness)
            cv2.putText(img, "Left", (160, 220), cv2.FONT_HERSHEY_SIMPLEX, font_size, font_color, font_thickness)
            cv2.putText(img, "Car:        "+str(right_list[0])+"     "+ str(left_list[0]), (20, 240), cv2.FONT_HERSHEY_SIMPLEX, font_size, font_color, font_thickness)
            cv2.putText(img, "Motorbike:  "+str(right_list[1])+"     "+ str(left_list[1]), (20, 260), cv2.FONT_HERSHEY_SIMPLEX, font_size, font_color, font_thickness)
            cv2.putText(img, "Bus:        "+str(right_list[2])+"     "+ str(left_list[2]), (20, 280), cv2.FONT_HERSHEY_SIMPLEX, font_size, font_color, font_thickness)
            cv2.putText(img, "Truck:      "+str(right_list[3])+"     "+ str(left_list[3]), (20, 300), cv2.FONT_HERSHEY_SIMPLEX, font_size, font_color, font_thickness)
            # Show the frames
